...
struct from_palette: base {
  ...
  sampled_spectrum sample(point2 const &, sampled_wl const &wl) const {
    sampled_spectrum s;
    for (int i = 0; i < nb_s; ++i) {
      int l = palette_channels * (wl.lambda[i] - min_wl) / (max_wl - min_wl);
...
std::array<double, palette_channels> const palY = []() {
  std::array<double, palette_channels> r;
  r.fill(0.);
  for (int w = min_wl; w < max_wl; ++w) {
    int l = palette_channels * ((double)w - min_wl) / (max_wl - min_wl);
    r[l] += toY(w);
  }
  return r;
 }();
...
sampled_spectrum fromXYZ(vec const &c, sampled_wl const &wl) {
  ...
  int val = palette[y][x];
  double f = 0;
  for (int l = 0; l < palette_channels; ++l) {
    f += (3 & (val >> (2 * l))) * palY[l] / 3;
  }
  if (c[1] >= f) f = 1;
  else f = c[1] / f;
  sampled_spectrum s;
